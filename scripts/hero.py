...
class Hero:
    ...
    def load_heroes():
        ...
        for line in json_lines:
            ...
            Hero.heroes[new_hero.dbf_id] = new_hero
        Hero.setup_valid_heroes()

    def pull_heroes(num_heroes):
        heroes = []
        for i in range(num_heroes):
            new_dbf = Hero.valid_hero_dbfs[random.randint(0, 
             len(Hero.valid_hero_dbfs) - 1)]
            heroes.append(Hero.heroes[new_dbf])
            Hero.taken_hero_dbfs.append(new_dbf)
            Hero.valid_hero_dbfs.remove(new_dbf)
        return heroes
